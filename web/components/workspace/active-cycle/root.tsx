...
export const WorkspaceActiveCycleRoot: React.FC<IActiveCycleDetails> = observer((props) => {
    ...
    const { isLoading } = useSWR(
        ...
        workspaceSlug
            ? async () => {
                  const response = await fetchActiveWorkspaceCycles(workspaceSlug)
            ...
            {Object.keys(workspaceCycles).map((projectId) => {
                workspaceCycles[projectId].map((activeCycle) => {
                    ...
                    const { data: activeCycleIssues } = useSWR(
                        workspaceSlug && projectId && activeCycle.id
                            ? CYCLE_ISSUES_WITH_PARAMS(activeCycle.id, { priority: "urgent,high" })
                            : null,
                        workspaceSlug && projectId && activeCycle.id
                            ? () => fetchActiveCycleIssues(workspaceSlug, projectId, activeCycle.id)
                            : null
                    )
                                                        ...
                                                        title={
                                                            <div className="flex items-center gap-2">
                                                                <span
                                                                    ...
                                                                    style={{
                                                                        backgroundColor:
                                                                            CYCLE_STATE_GROUPS_DETAILS[index]?.color,
                                                                    }}
                                                                />
