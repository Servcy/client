...
export const CalendarWeekHeader: React.FC<Props> = observer((props) => {
    ...
    return (
        <div
            className={`sticky top-0 z-[1] grid divide-x-[0.5px] divide-custom-border-200 text-sm font-medium ${
        ...
        >
            ...
            {Object.values(DAYS_LIST).map((day) => {
                ...
                return (
                    <div
                        key={day.shortTitle}
                        className="flex h-11 items-center justify-end bg-custom-background-90 px-4"
                    >
                        {day.shortTitle}
                    </div>
                )
