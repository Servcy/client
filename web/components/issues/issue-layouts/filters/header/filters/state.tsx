...
export const FilterState: React.FC<Props> = observer((props) => {
    ...
    const sortedOptions = useMemo(() => {
        const _filteredOptions = (states ?? []).filter((s) => s.name.toLowerCase().includes(searchQuery.toLowerCase()))

        return sortBy(_filteredOptions, [(s) => !(appliedFilters ?? []).includes(s.id), (s) => s.name.toLowerCase()])
