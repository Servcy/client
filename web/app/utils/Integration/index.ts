const getMicrosoftOauthUrl = (from: string) => {
    ...
    const options = [
        ...
        ["response_type", "code"],
        ["redirect_uri", `${process.env["NEXT_PUBLIC_CLIENT_URL"]}/integrations/outlook/oauth`],
        ["state", from],
        ["scope", scopes.join(" ")],
        ["response_mode", "query"],
        ["prompt", "consent"],
    ]
    return `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?${new URLSearchParams(options)}`
}

const getGoogleOauthUrl = (from: string) => {
    const options = [
        ["redirect_uri", `${process.env["NEXT_PUBLIC_CLIENT_URL"]}/integrations/gmail/oauth`],
...
const getJiraOauthUrl = (from: string) =>
    `https://auth.atlassian.com/authorize?${new URLSearchParams([
        ...
        ["scope", ["read:me", "read:jira-work", "write:jira-work", "manage:jira-webhook", "offline_access"].join(" ")],
        ["state", from],
        ["response_type", "code"],
        ["prompt", "consent"],
    ])}`
