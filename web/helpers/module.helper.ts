...
import { IModule, TModuleDisplayFilters, TModuleFilters, TModuleOrderByOptions } from "@servcy/types"
        ...
        if (filterKey === "members" && filters.members && filters.members.length > 0)
            fallsInFilters =
                ...
                filters.members.some((memberId) =>
                    module.member_ids.map((member_id) => member_id.toString()).includes(memberId.toString())
                )
        if (filterKey === "start_date" && filters.start_date && filters.start_date.length > 0) {
            filters.start_date.forEach((dateFilter) => {
                fallsInFilters =
                    fallsInFilters &&
                    !!module.start_date &&
                    satisfiesDateFilter(new Date(module.start_date), dateFilter)
            })
        }
        if (filterKey === "target_date" && filters.target_date && filters.target_date.length > 0) {
            filters.target_date.forEach((dateFilter) => {
                fallsInFilters =
                    fallsInFilters &&
                    !!module.target_date &&
                    satisfiesDateFilter(new Date(module.target_date), dateFilter)
            })
        }
    })
