  ...
  return (
    <div className="col-span-2 max-h-[600px] overflow-y-scroll rounded-l-lg bg-servcy-black p-4 text-servcy-white">
      {["FILE_UPDATE", "FILE_DELETE"].includes(
        data.event_type
      ) ? null : data.event_type === "LIBRARY_PUBLISH" ? (
        <div className="min-h-[75px]">
          {data.description && (
            <div className="mb-2 font-normal"> {data.description}</div>
          )}
        ...
        <div className="min-h-[75px]">
          {data.comment.map((comment: Comment, index1: number) => {
            if (comment.text) {
                ...
                <span key={index1}>
                  {lines.map((line, index2) => (
                    <>
                      ...
                      {data.comment && index1 !== data.comment.length - 1
                        ? " "
                        : null}
                    </>
                  ))}
              ...
              const handle = data.mentions.find(
                (mention: Mention) => mention.id === comment.mention
              );
              if (handle) {
                return (
                  <>
